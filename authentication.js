...
const createUser = (req, res) => {
  ...
  const { uname, upassword, firstname, email, mobilenumber } = req.body;
  const insertUser =
    "insert into patient (uname,upassword,firstname,email,mobilenumber) values ( $1, $2, $3, $4,$5)";
  console.log(insertUser);
  client.query(
    insertUser,
    [uname, upassword, firstname, email,mobilenumber],
    (err, response) => {
      if (!err) {
...
const loginUser = (req, res) => {
  ...
  const checkcrendtials =
    "select * from patient where uname = $1 and upassword = $2";
